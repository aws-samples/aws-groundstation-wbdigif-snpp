...
systemctl stop blink-config.service
...
cp ${tmpDir}/modem_ami/config_service/blink-config.service /etc/systemd/system/
...
echo -e "\nFetching auth token ..."
bearerToken=$(curl -o ${tmpDir}/headers.txt -X POST https://localhost/mnc/rest/security/login -u "blink-admin:blink-admin" -H "Content-Type: application/json" -H "Content-Length: 0" -k --head && cat ${tmpDir}/headers.txt | awk '/Authorization/ {printf $3}')
